...
#Hay que mejorarla
def num_activations(entries, eidx_to_clabel, policy):

    matches = []
    activations = []
    
    #For each entry count the num of acts in the policy
    for eidx,entry in enumerate(entries):
        
        denials_count = 0
        
        for ridx,rule in enumerate(policy):
            ...
            if eidx_to_clabel[eidx] == ridx:
                if not granted:
                    matches.append(False)
                else:
                    matches.append(True)
                
        activations.append(len(policy)-denials_count)
        
    return matches,activations
